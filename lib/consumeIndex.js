...
function findTextNodes(nodeList) {
  return Array.from(nodeList).filter((node) => node.nodeType === 3);
}

export default function consumeIndex(document) {
  return Array.from(document.querySelectorAll(".sectiontable tr"))
    ...
    .map((row) => {
      ...
      const extraHeadings = Array.from(
        row.querySelectorAll(".col3 > span > u, .col7 > span > u")
      );
      ...
      if (extraHeadings.length) {
        extraHeadings.forEach((heading) => {
          ...
          const headingDetailEntries = findTextNodes(
            heading.nextElementSibling.childNodes
          ).map((text) => text.textContent.replace(/[\s\n]+/g, " ").trim());
